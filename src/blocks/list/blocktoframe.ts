...
const setItemInList = (
  ...
  const explanation = `Тізім "${newVariable.name}" ${stringValue}-ды ${position + 1}-орында сақтайды.`;
...
const createExplanation = (variable: Variable, size: number) => {
  return `${variable.name}" деп аталатын ${typeToHumanWord(variable.type)} айнымалысын жасап, ${size} элементті сақтайды.`
